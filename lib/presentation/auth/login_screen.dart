...
class _LoginScreenState extends ConsumerState<LoginScreen> {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            const Center(child: HaroAvatar(size: 96, mood: HaroMoodVisual.thinking)),
            ...
            Row(
              children: [
                Checkbox(value: _remember, onChanged: (value) => setState(() => _remember = value ?? true)),
            ...
            const SizedBox(height: 8),
            AppButton(label: 'Masuk dengan Google', icon: Icons.g_mobiledata, isSecondary: true, onPressed: () {}),
            TextButton(
  ...
  Future<void> _login() async {
    final ok = await ref.read(appStateProvider.notifier).login(_email.text.trim(), _password.text);
    if (!mounted) return;
    if (ok) {
